...
void handleRoot() {
    ...
    page.replace("%POOL_PASSWORD%", processor("POOL_PASSWORD"));
...
void handleSave() {
    ...
    if (wifi_ssid.length() > 0 && btc_address.length() > 0) {
        ...
        config.btc_address[sizeof(config.btc_address) - 1] = '\0';
        strncpy(config.pool_url, pool_url.c_str(), sizeof(config.pool_url) - 1);
        config.pool_url[sizeof(config.pool_url) - 1] = '\0';
        config.pool_port = pool_port;
        strncpy(config.pool_password, pool_password.c_str(), sizeof(config.pool_password) - 1);
        config.pool_password[sizeof(config.pool_password) - 1] = '\0';
        config.configured = true;
        saveConfig();
        File success = SPIFFS.open("/success.html", "r");
        if (success) {
            String success_html = success.readString();
            success.close();
...
void saveConfig() {
    ...
    preferences.putString("pool_password", config.pool_password);
...
void loadConfig() {
    ...
    if (config.configured) {
        ...
        strncpy(config.wifi_ssid, stored_ssid.c_str(), sizeof(config.wifi_ssid) - 1);
        config.wifi_ssid[sizeof(config.wifi_ssid) - 1] = '\0';

        String stored_wifi_pass = preferences.getString("wifi_password", "");
        strncpy(config.wifi_password, stored_wifi_pass.c_str(), sizeof(config.wifi_password) - 1);
        config.wifi_password[sizeof(config.wifi_password) - 1] = '\0';

        String stored_btc = preferences.getString("btc_address", DEFAULT_ADDRESS);
        strncpy(config.btc_address, stored_btc.c_str(), sizeof(config.btc_address) - 1);
        config.btc_address[sizeof(config.btc_address) - 1] = '\0';

        String stored_pool_url = preferences.getString("pool_url", DEFAULT_POOL_URL);
        strncpy(config.pool_url, stored_pool_url.c_str(), sizeof(config.pool_url) - 1);
        config.pool_url[sizeof(config.pool_url) - 1] = '\0';

        config.pool_port = preferences.getInt("pool_port", DEFAULT_POOL_PORT);

        String stored_pool_pass = preferences.getString("pool_password", DEFAULT_POOL_PASSWORD);
        strncpy(config.pool_password, stored_pool_pass.c_str(), sizeof(config.pool_password) - 1);
        config.pool_password[sizeof(config.pool_password) - 1] = '\0';
    }
}
